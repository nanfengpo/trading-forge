...
def _fetch_finnhub_metrics(ticker: str) -> Optional[Dict[str, Any]]:
    """Finnhub /stock/metric — used as a fallback for Quarterly YoY growth
    when AV OVERVIEW lacks the field. Returns subset of basic metrics.
    """
    key = _finnhub_key()
    if not key:
        return None
    try:
        ...
        return {
            "fh_rev_growth_ttm_yoy":  _safe_float(m.get("revenueGrowthTTMYoy")),
            "fh_eps_growth_ttm_yoy":  _safe_float(m.get("epsGrowthTTMYoy")),
...
def _normalise_row(ticker: str, ov: Optional[Dict[str, Any]],
    ...
    """Flatten OVERVIEW + Polygon + Finnhub into the row our UI needs.
    ...
    if not ov:
        ...
        # If AV failed but Polygon worked we can still show price.
        if poly:
            base["price"] = poly.get("price")
            base["change_pct"] = poly.get("change_pct")
        return base
    ...
    # Cross-check with Finnhub TTM YoY when AV is missing the value.
    # Finnhub's revenueGrowthTTMYoy / epsGrowthTTMYoy are already % values.
    if fh:
        if rev_g is None and fh.get("fh_rev_growth_ttm_yoy") is not None:
            rev_g = fh["fh_rev_growth_ttm_yoy"]
        if eps_g is None and fh.get("fh_eps_growth_ttm_yoy") is not None:
            eps_g = fh["fh_eps_growth_ttm_yoy"]
    ...
    # Pull live price from Polygon when present; fall back to AV's last close
    # if we have it (AV OVERVIEW doesn't include a current price, only 52w).
    price = (poly or {}).get("price")
    change_pct = (poly or {}).get("change_pct")
    ...
    return {
        ...
        # Cross-source diagnostics — useful for QA, hidden from default UI
        "_fh_eps_growth_5y": (fh or {}).get("fh_eps_growth_5y"),
        "_fh_beta":          (fh or {}).get("fh_beta"),
    }
...
def _fetch_one(ticker: str, force: bool = False) -> Dict[str, Any]:
    ...
    # Only cache when we got real fundamentals data, so transient failures retry.
    if ov is not None:
...
def fetch_sector_rows(sector_id: str, force: bool = False) -> List[Dict[str, Any]]:
    ...
    pulls live data from AV / Polygon / Finnhub.
    """
    ...
    def _wrapped(t: str) -> Dict[str, Any]:
        ...
        # Always stamp sub_sector (it's defined by our taxonomy, not the vendor).
...
def build_commentary(sector: Dict[str, Any],
    ...
    judge = "偏高" if (median is not None and median < 45) else \
            "中性" if (median is not None and median < 60) else \
            "中性偏强" if (median is not None and median < 70) else "偏强"
    paragraphs.append({
        "title": "整体估值水位",
        "body": (
            f"<strong>{sector_name}</strong> 覆盖 <strong>{n_total}</strong> 只股票，有评分的 "
            f"<strong>{n_scored}</strong> 只，板块综合分中位数 <strong>{median_str}</strong>，"
            f"整体估值<strong>{judge}</strong>。"
            f"分级分布：优秀 (≥70) <strong>{good}</strong> 只，中性 (45–70) <strong>{mid}</strong> 只，"
            f"弱势 (&lt;45) <strong>{high}</strong> 只。"
            f"评分基于本板块自适应权重（"
            f"PE {sector['weights']['pe']}% · Fwd PE {sector['weights']['pe_fwd']}% · "
            f"PEG-AV {sector['weights']['peg_av']}% · PEG-Fwd {sector['weights']['peg_fwd']}% · "
            f"营收 {sector['weights']['rev_growth']}% · 利润 {sector['weights']['eps_growth']}%）。"
        ),
    })
...
def build_sector_payload(sector_id: str, force: bool = False) -> Dict[str, Any]:
    ...
    `force=True` bypasses the per-ticker TTL cache so a manual refresh
    always pulls fresh data from Alpha Vantage / Polygon / Finnhub.
    """
