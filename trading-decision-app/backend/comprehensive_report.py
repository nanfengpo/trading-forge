...
6. **horizons.{short|mid|long}.strategies** 必须是**数组**，**每个时段至少 3 条**，按"匹配度（与当前观点和市场条件的契合度）"从高到低排序。每条策略必须包含：
   - `name`：策略名称；若 decisions_newest_first[*].matched_strategies 里有合适匹配项，**直接引用其 name**；否则给出明确策略名（例如"分批做多 + 跌破止损"、"看涨价差 (Bull Call Spread)"、"网格区间套利"）。
   - `match_score`：`high|medium|low`，反映该策略与当前观点的契合度。
   - `thesis`：1-2 句说明此刻为何采用该策略（结合 trend / 目标价 / 风险事件）。
   - `execution`：**完整可执行操作**：建仓节奏（具体分批价位）+ 仓位（%NAV）+ 止损价 + 减仓 / 加仓阶梯 + 需要持续盯的信号 / 数据节点（3-5 句，必须含具体数字）。
   严禁出现单数 `strategy` 字段；旧字段已废弃。
7. **horizons.long** 是 **6-12 个月**（不是"6 个月以上"）—— 给出可预测、可执行的目标价区间与策略，而非无限远的展望。
...

{
  ...
  "horizons": {
    "short": {
      ...
      "strategies": [
        {
          "name": "策略名（优先引用 matched_strategies 中的 name）",
          "match_score": "high|medium|low",
          "thesis": "1-2 句：此时此景为什么用这条策略",
          "execution": "完整执行操作：分批价位 + 仓位 %NAV + 止损价 + 减仓/加仓阶梯 + 需盯信号（3-5 句，含具体数字）"
        },
        "至少 3 条；按匹配度从高到低排序"
      ],
